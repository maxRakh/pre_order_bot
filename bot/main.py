...
async def start(message: types.Message) -> None:
    if message.from_user.id in users:
        start_buttons = ["Предзаказы выгрузить", "Предазказы товара", "Добавить предзаказ", "Выгрузить предзаказы в   "]
...
async def inline_keyboard_preorders_buttons(callback: types.CallbackQuery) -> None:
    ...
    if callback.data == 'all_preorders':
        ...
        data = api.get_all_data("preorder", page_number=page_number, page_size=page_size, bought=False, canceled=False)
    elif callback.data == 'all_bought_preorders':
        data = api.get_all_data("preorder", page_number=page_number, page_size=page_size, bought=True)
    elif callback.data == 'all_canceled_preorders':
        data = api.get_all_data("preorder", page_number=page_number, page_size=page_size, canceled=True)
